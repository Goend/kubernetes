...
func containerChanged(container *v1.Container, containerStatus *kubecontainer.Status, skipfn func() bool) (uint64, uint64, bool) {
	expectedHash := kubecontainer.HashContainer(container)
	if skipfn != nil && skipfn() {
		return expectedHash, containerStatus.Hash, false
	}
...
func (m *kubeGenericRuntimeManager) computePodActions(ctx context.Context, pod *v1.Pod, podStatus *kubecontainer.PodStatus) podActions {
	...
	for idx, container := range pod.Spec.Containers {
		...
		if _, _, changed := containerChanged(&container, containerStatus, m.runtimeHelper.SkipContainerHash); changed {
